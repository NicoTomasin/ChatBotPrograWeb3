...
public class PeliculaRandom : ComponentDialog
{
    ...
    public PeliculaRandom() : base(nameof(PeliculaRandom))
    {
    ...
    private async Task<DialogTurnResult> ShowPeliculasDelDiaAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
    {
        ...
        var heroCard = await RandomMovieAsync(genre);
    ...
    private async Task<HeroCard> RandomMovieAsync(string genre)
    {
        ...
        if (movies.Any())
        {
            ...
            var randomMovie = movies[random.Next(0, movies.Length)];

            var title = randomMovie["title"].ToString();
            var posterPath = randomMovie["poster_path"].ToString();
